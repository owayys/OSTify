            ...
            spotifyApi
                ...
                .then(async (audioFeatures) => {
                    ...
                    for (const genre of genres) {
                        genre_embeddings[genre] = await generateEmbeddings(
                            genre,
                            {
                                pooling: "mean",
                                normalize: true,
                    ...
                    for (let i = 0; i < 3; i++) {
                        seed_tracks.push(
                            genre_seeds[top_genres[0]][
                                Math.floor(
                                    Math.random() *
                                        genre_seeds[top_genres[0]].length
                                )
                            ]
                        );
                    }
                    seed_tracks.push(
                        genre_seeds[top_genres[1]][
                            Math.floor(
                                Math.random() *
                                    genre_seeds[top_genres[1]].length
                            )
                        ]
                    );
                    seed_tracks.push(
                        genre_seeds[top_genres[2]][
                            Math.floor(
                                Math.random() *
                                    genre_seeds[top_genres[2]].length
                            )
                        ]
                    );
                    ...
                    spotifyApi
                        .getRecommendations({
                            ...
                            target_acousticness:
                                top_track_features?.acousticness?.mean || 0.3,
                            target_energy:
                                top_track_features?.energy?.mean || 0.3,
                            target_instrumentalness:
                                top_track_features?.instrumentalness?.mean +
                                    top_track_features?.instrumentalness
                                        ?.standardDeviation /
                                        2 || 0.7,
                            target_danceability:
                                top_track_features?.danceability?.mean || 0.3,
                            target_speechiness:
                                top_track_features?.speechiness?.mean -
                                    top_track_features?.speechiness
                                        ?.standardDeviation /
                                        2 || 0.15,
                            target_loudness:
                                top_track_features?.loudness?.mean || 0.3,
                            target_tempo: top_track_features?.tempo?.mean || 80,
                            target_valence:
                                top_track_features?.valence?.mean || 0.5,
                        })
                        .then((response) => {
                            ...
                            spotifyApi.getMe().then((user) => {
                                spotifyApi
                                    ...
                                    .then((playlist) => {
                                        const uris = response.tracks.map(
                                            (track) => track.uri
                                        );
                                        spotifyApi
                                            .addTracksToPlaylist(
